...
def select_region(image):
    ...
    bottom_left  = [cols*0, rows*0.6] #0 0.6
    top_left     = [cols*0.1, rows*0.45] #0.1 0.45
    bottom_right = [cols*1, rows*0.6] # 1 0.6
    top_right    = [cols*0.9, rows*0.45] #0.9 0.45
...
while True:
        ...
        list_of_lines        = hough_lines(regions)

        if list_of_lines is None:
                font = cv2.FONT_HERSHEY_SIMPLEX
                image=cv2.putText(image,'No line detected',(10,500), font, 4,(255,255,255),2,cv2.LINE_AA)
        else:
                for lists in list_of_lines:
                     for lines in lists:
                            img= cv2.line(image,(lines[0],lines[1]),(lines[2],lines[3]),(0,0,255),5)
        left_line, right_line = lane_lines(image, list_of_lines)
        '''
